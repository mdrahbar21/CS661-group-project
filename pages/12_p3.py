...
BATSMAN_METRICS = OrderedDict([
    ('total_runs', {'label': 'Total Runs',
     'higher_is_better': True, 'format': ',.0f'}),
    ('average', {'label': 'Batting Average',
     'higher_is_better': True, 'format': '.2f'}),
    ('strike_rate', {'label': 'Strike Rate',
     'higher_is_better': True, 'format': '.2f'}),
    ('total_fours', {'label': 'Total Fours',
     'higher_is_better': True, 'format': ',.0f'}),
    ('total_sixes', {'label': 'Total Sixes',
     'higher_is_better': True, 'format': ',.0f'}),
    ('innings', {'label': 'Innings Played',
     'higher_is_better': True, 'format': ',.0f'}),
])
...
BOWLER_METRICS = OrderedDict([
    ('wickets', {'label': 'Total Wickets',
     'higher_is_better': True, 'format': ',.0f'}),
    ('bowling_average', {'label': 'Bowling Average',
     'higher_is_better': False, 'format': '.2f'}),  # Lower is better
    ('economy', {'label': 'Economy Rate', 'higher_is_better': False,
     'format': '.2f'}),  # Lower is better
    ('bowling_strike_rate', {'label': 'Bowling Strike Rate',
     'higher_is_better': False, 'format': '.2f'}),  # Lower is better
    ('matches_bowled', {'label': 'Matches Bowled',
     'higher_is_better': True, 'format': ',.0f'}),
])
...
TEAM_METRICS = OrderedDict([
    ('win_percentage', {'label': 'Win Percentage',
     'higher_is_better': True, 'format': '.1f'}),
    ('total_wins', {'label': 'Total Wins',
     'higher_is_better': True, 'format': ',.0f'}),
    ('matches_played', {'label': 'Matches Played',
     'higher_is_better': True, 'format': ',.0f'}),
])
...
layout = dbc.Container([
    ...
    dbc.Card(
        dbc.CardBody([
            dbc.Row([
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[
                            {'label': 'All International (ODI, T20I, Test)',
                             'value': 'All International'},
                            {'label': 'ODI', 'value': 'ODI'},
                            {'label': 'T20I', 'value': 'T20I'},
                            {'label': 'Test', 'value': 'Test'},
                            # Include domestic T20 etc.
            ...
            dbc.Row(dbc.Col(
                html.Small(id='atr-qualification-info',
    ...
    html.Div(id='atr-visualizations', children=[
        dbc.Row([
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody([
                        html.H5(id='atr-bar-chart-title',
                                className="card-title text-center"),
                        # Initialize with empty figure
                        dcc.Loading(
                            dcc.Graph(id='atr-ranking-bar-chart', figure=go.Figure()))
                    ])
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody([
                        html.H5(id='atr-scatter-plot-title',
                                className="card-title text-center"),
                        # Use the correct ID here
                        dcc.Loading(
                            dcc.Graph(id='atr-ranking-scatter-plot', figure=go.Figure()))
                    ])
                ), width=12, lg=6, className="mb-3"  # Use half width on large+ screens
            )
    ...
    dbc.Row([
        dbc.Col(
            dbc.Card(
                dbc.CardBody([
                    html.H4(
                        f"Detailed Ranking Table (Top {TOP_N})", className="text-center mb-3"),
                    dcc.Loading(
...
def update_rankings_and_plots(category, metric, match_type_filter, stored_main_data):
    ...
    print(
        f"ATR Callback: Updating table & plots for Category={category}, Metric={metric}, Filter={match_type_filter}")
    ...
    no_data_fig = create_empty_figure(
        "No qualifying data found for these selections.")
    ...
    if not metric_config:
        print(
            f"ATR Error: Invalid metric '{metric}' for category '{category}'.")
        error_cols = [{"name": "Error", "id": "error"}]
        error_data = [{"error": f"Invalid Metric '{metric}' for {category}"}]
        error_fig_metric = create_empty_figure(
    ...
    # Use empty string if no format
    metric_format = metric_config.get('format', '')
    ...
    if 'match_type' in df_filtered.columns:
        if match_type_filter == 'All International':
            ...
            df_filtered = df_filtered[df_filtered['match_type'].astype(
                str).isin(allowed_types)]
        elif match_type_filter != 'All Formats':  # Specific format selected
            df_filtered = df_filtered[df_filtered['match_type'].astype(
                str) == match_type_filter]
    ...
    if df_filtered.empty:
        print(
            f"ATR Info: No data found after filtering for match type: {match_type_filter}")
        no_data_cols = [{"name": "Info", "id": "info"}]
        no_data_data = [
            {"info": f"No data found for Match Type: {match_type_filter}"}]
        no_data_fig_filter = create_empty_figure(
            f"No data for {match_type_filter}")
    ...
    try:
        if category == 'Batsman':
            ...
            player_stats = pd.merge(
                player_stats, innings_count, on='name', how='left').fillna({'innings': 0})
            # Use copy to avoid SettingWithCopyWarning
            player_stats = player_stats[player_stats['innings']
                                        >= MIN_INNINGS_BATTED].copy()

            if player_stats.empty:
                raise ValueError(
                    f"No batsmen qualify with >= {MIN_INNINGS_BATTED} innings.")

            # Calculate derived metrics carefully checking for division by zero
            player_stats['average'] = np.where(
                player_stats['outs'] > 0, player_stats['total_runs'] / player_stats['outs'], np.nan)
            player_stats['strike_rate'] = np.where(player_stats['balls_faced'] > 0, (
                player_stats['total_runs'] / player_stats['balls_faced']) * 100, np.nan)
            ...
            req_cols = ['name', 'match_id', 'wickets_taken',
                        'runs_conceded', 'balls_bowled']
            if not all(c in df_filtered.columns for c in req_cols):
                missing = [c for c in req_cols if c not in df_filtered.columns]
                raise ValueError(
                    f"Missing required columns for Bowler analysis: {missing}")

            df_filtered.dropna(subset=['name'], inplace=True)
            if df_filtered.empty:
                raise ValueError("No valid bowler data after filtering.")
            ...
            player_stats = pd.merge(player_stats, matches_bowled_count,
                                    on='name', how='left').fillna({'matches_bowled': 0})
            player_stats = player_stats[player_stats['balls_bowled']
                                        >= MIN_BALLS_BOWLED].copy()

            if player_stats.empty:
                raise ValueError(
                    f"No bowlers qualify with >= {MIN_BALLS_BOWLED} balls bowled.")

            player_stats['bowling_average'] = np.where(
                player_stats['wickets'] > 0, player_stats['runs_conceded'] / player_stats['wickets'], np.nan)
            player_stats['economy'] = np.where(player_stats['balls_bowled'] > 0, (
                player_stats['runs_conceded'] / player_stats['balls_bowled']) * 6, np.nan)
            player_stats['bowling_strike_rate'] = np.where(
                player_stats['wickets'] > 0, player_stats['balls_bowled'] / player_stats['wickets'], np.nan)
            ...
            if not all(c in df_filtered.columns for c in req_cols):
                missing = [c for c in req_cols if c not in df_filtered.columns]
                raise ValueError(
                    f"Missing required columns for Team analysis: {missing}")

            # Ensure team names are consistent and handle NaNs
            df_filtered['batting_team'] = df_filtered['batting_team'].astype(
                str)
            df_filtered['bowling_team'] = df_filtered['bowling_team'].astype(
                str)
            df_filtered['winner'] = df_filtered['winner'].astype(str)
            df_filtered.dropna(
                subset=['batting_team', 'bowling_team', 'winner', 'match_id'], inplace=True)

            match_info = df_filtered.drop_duplicates(subset='match_id')[
                ['match_id', 'winner']].copy()
            teams_in_matches = pd.concat([
                df_filtered[['match_id', 'batting_team']].rename(
                    columns={'batting_team': entity_name_col}),
                df_filtered[['match_id', 'bowling_team']].rename(
                    columns={'bowling_team': entity_name_col})
            ]).drop_duplicates()
            # Filter out potential placeholder/NaN team names if needed
            teams_in_matches = teams_in_matches[teams_in_matches[entity_name_col].str.lower(
            ) != 'nan']

            if teams_in_matches.empty:
                raise ValueError("No valid team data found.")

            team_stats = pd.merge(
                teams_in_matches, match_info, on='match_id', how='left')
            team_stats['is_winner'] = np.where(
                team_stats[entity_name_col] == team_stats['winner'], 1, 0)
            ...
            agg_stats = agg_stats[agg_stats['matches_played']
                                  >= MIN_MATCHES_PLAYED].copy()

            if agg_stats.empty:
                raise ValueError(
                    f"No teams qualify with >= {MIN_MATCHES_PLAYED} matches played.")

            agg_stats['win_percentage'] = np.where(agg_stats['matches_played'] > 0, (
                agg_stats['total_wins'] / agg_stats['matches_played']) * 100, 0.0)

            required_columns_map = {
                # Use the entity name col here
                'Rank': {'label': 'Rank'}, entity_name_col: {'label': 'Team'},
                'win_percentage': {'label': 'Win %', 'format': '.1f'},
                'total_wins': {'label': 'Wins', 'format': ',.0f'},
                'matches_played': {'label': 'Played', 'format': ',.0f'},
            }
            # Rename to 'name' for internal consistency AFTER map defined
            df_ranked = agg_stats.rename(columns={entity_name_col: 'name'})
        ...
        if metric not in df_ranked.columns:
            ...
            raise ValueError(
                f"Metric column '{metric}' not found after aggregation for {category}.")

        # Drop rows where the ranking metric itself is NaN (essential for ranking)
        df_ranked.dropna(subset=[metric], inplace=True)
        if df_ranked.empty:
            raise ValueError(
                f"No players/teams remain after removing NaN values for ranking metric '{metric_label}'.")

        ascending_order = not metric_config.get(
            'higher_is_better', True)  # Default to True if missing
        df_ranked.sort_values(
            by=metric, ascending=ascending_order, inplace=True, na_position='last')
        ...
        dt_columns = []
        display_cols_order = list(
            required_columns_map.keys())  # Get order from map
        ...
        if 'team_name' in display_cols_order and category == 'Team':
            ...
            required_columns_map['name'] = required_columns_map.pop(
                'team_name')

        df_display_table = df_top_n_table[[
            col for col in display_cols_order if col in df_top_n_table.columns]]
        ...
        graph_entity_label = required_columns_map.get(
            entity_name_col, {}).get('label', 'Entity')

        # --- Create Bar Chart ---
        bar_fig = create_empty_figure(
            f"Not enough data for {metric_label} bar chart.")
        ...
        if not df_graph_data.empty:
            try:
                ...
                bar_hover_cols = [col for col in required_columns_map if col in df_graph_data.columns and col not in [
                    entity_name_col, metric, 'Rank']]
                custom_data_bar = df_graph_data[bar_hover_cols]

                # Construct the hover template string
                # Use customdata[0] for name
                hovertemplate_bar = f"<b>%{{customdata[0]}}</b><br><br>"
                # Main metric (y-axis)
                hovertemplate_bar += f"{metric_label}=%{{y:{metric_format}}}<br>"
                ...
                for i, col in enumerate(bar_hover_cols[1:], 2):
                    label = required_columns_map[col]['label']
                    fmt = required_columns_map[col].get(
                        'format', '')  # Get format specifier
                ...
                bar_custom_data_cols = [
                    entity_name_col, 'Rank'] + bar_hover_cols[1:]
                ...
                bar_fig = px.bar(
                    ...
                    y=metric,
                    title=None,
                    labels={entity_name_col: graph_entity_label,
                            metric: metric_label},
                    text=metric,
                ...
                bar_fig.update_layout(
                    xaxis={'categoryorder': 'total descending' if metric_config.get(
                        'higher_is_better', True) else 'total ascending'},
                ...
                bar_fig = create_empty_figure(
                    f"Error generating bar chart for {metric_label}")

        # --- Create Scatter Plot ---
        scatter_fig = create_empty_figure("Cannot generate scatter plot.")
        scatter_title = "Comparison Plot"
        sec_metric_key, sec_metric_label = SECONDARY_SCATTER_METRIC.get(
            category, {}).get(metric, (None, None))
        ...
        if sec_metric_key and sec_metric_key in df_graph_data.columns:
            ...
            color_metric_format = ''
            if category == 'Batsman' and 'innings' in df_graph_data.columns:
                color_metric, color_label = 'innings', get_metric_label(
                    'Batsman', 'innings')
            elif category == 'Bowler' and 'matches_bowled' in df_graph_data.columns:
                color_metric, color_label = 'matches_bowled', get_metric_label(
                    'Bowler', 'matches_bowled')
            elif category == 'Team' and 'matches_played' in df_graph_data.columns:
                color_metric, color_label = 'matches_played', get_metric_label(
                    'Team', 'matches_played')
            if color_metric:
                color_metric_format = get_metric_config(
                    category, color_metric).get('format', '')

            # Drop NaNs for scatter plot axes AND color metric before proceeding
            scatter_req_cols = [metric, sec_metric_key]
            if color_metric:
                scatter_req_cols.append(color_metric)
            df_graph_data_scatter = df_graph_data.dropna(
                subset=scatter_req_cols).copy()
            ...
            if not df_graph_data_scatter.empty:
                ...
                scatter_exclude_cols = [
                    entity_name_col, metric, sec_metric_key, 'Rank']
                if color_metric:
                    scatter_exclude_cols.append(color_metric)

                scatter_hover_cols = [
                    col for col in required_columns_map if col in df_graph_data_scatter.columns and col not in scatter_exclude_cols]

                # Construct the scatter hover template
                # Name
                hovertemplate_scatter = f"<b>%{{customdata[0]}}</b><br><br>"
                hovertemplate_scatter += f"Rank=%{{customdata[1]}}<br>"  # Rank
                # x-axis
                hovertemplate_scatter += f"{metric_label}=%{{x:{metric_format}}}<br>"
                # y-axis
                hovertemplate_scatter += f"{sec_metric_label}=%{{y:{sec_metric_format}}}<br>"
                if color_metric:
                    ...
                    # color axis value
                    hovertemplate_scatter += f"{color_label}=%{{customdata[2]:{color_metric_format}}}<br>"
                ...
                if color_metric:
                    scatter_custom_data_cols.append(color_metric)
                ...
                try:
                    scatter_fig = px.scatter(
                        ...
                        title=None,
                        labels={
                            metric: metric_label, sec_metric_key: sec_metric_label, color_metric: color_label},
                        custom_data=scatter_custom_data_cols,  # Pass list of columns
                        color=color_metric,
                        color_continuous_scale=px.colors.sequential.Blues_r if category == 'Bowler' and (
                            # Reverse scale if lower is better for Y
                            'average' in sec_metric_key or 'economy' in sec_metric_key or 'strike_rate' in sec_metric_key) else px.colors.sequential.Blues,
                    ...
                    scatter_fig.update_layout(
                        ...
                        coloraxis_colorbar=dict(
                            title=color_label) if color_label else None,
                        margin=dict(t=20, b=10, l=10, r=10),
                        title=None
                    )
                    # Apply specific axis tick formatting
                    if metric_format:
                        scatter_fig.update_xaxes(tickformat=metric_format)
                    if sec_metric_format:
                        scatter_fig.update_yaxes(tickformat=sec_metric_format)

                except Exception as e:
                    print(f"ATR Error creating scatter plot: {e}")
                    traceback.print_exc()
                    scatter_fig = create_empty_figure(
                        f"Error generating plot for {metric_label} vs {sec_metric_label}")
                    scatter_title = f"Plot Error"
            else:
                scatter_fig = create_empty_figure(
                    f"Not enough valid data points for {metric_label} vs {sec_metric_label} plot.")
                scatter_title = f"{metric_label} vs {sec_metric_label} (No Data)"
        else:
            scatter_fig = create_empty_figure(
                f"No secondary metric defined or available for {metric_label}.")
            scatter_title = f"Comparison Plot (Not Available for {metric_label})"

        print(
            f"ATR Update: Successfully generated Top {len(df_top_n_table)} table and plots for {category} by {metric}")
        ...
        # Show viz area but with no data message
        return info_cols, info_data, info_fig, info_fig, "No Data", "No Data", visible_style
    except Exception as e:
        print(f"ATR Error during processing: {e}")
        traceback.print_exc()
        error_cols = [{"name": "Error", "id": "error"}]
        error_data = [
            {"error": "An error occurred during analysis. Check logs."}]
