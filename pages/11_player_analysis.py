...
dash.register_page(__name__, path='/player-analysis',
                   name='Player Analysis', title='Player Analysis')

# --- Configuration ---
# Suppress specific runtime warnings (e.g., divide by zero)
...
def calculate_batsman_stats(df):
    ...
    stats['innings'] = match_balls_faced[match_balls_faced >
                                         0].count()  # Count matches where player batted
    ...
    outs_per_match = df[df['player_out'].fillna(
        0) == 1].groupby('match_id').size()
    # Count matches where player got out
    stats['outs'] = int(outs_per_match.count())

    stats['average'] = (stats['runs'] / stats['outs']
                        ) if stats['outs'] > 0 else np.inf
    stats['strike_rate'] = (stats['runs'] / stats['balls_faced']
                            * 100) if stats['balls_faced'] > 0 else 0.0

    # Dismissal breakdown
    if 'out_kind' in df.columns:
        # Filter where player_out is 1 and out_kind is not NaN
        dismissals_df = df[(df['player_out'].fillna(0) == 1)
                           & (df['out_kind'].notna())]
    ...
    stats['not_outs'] = max(0, stats['innings'] -
                            stats['outs'])  # Ensure non-negative
...
def calculate_bowler_stats(df):
    ...
    match_bowling_stats = df.groupby('match_id').agg(
        ...
        dot_balls_as_bowler=('dot_balls_as_bowler', 'sum')
    ).reset_index()
    ...
    numeric_cols = ['balls_bowled', 'runs_conceded', 'wickets_taken',
                    'maidens', 'bowled_done', 'lbw_done', 'dot_balls_as_bowler']
    ...
    stats = {}
    # Matches player participated in overall
    stats['matches'] = df['match_id'].nunique()
    # Innings actually bowled
    stats['innings_bowled'] = bowling_innings.shape[0]
    ...
    stats['average'] = (stats['runs_conceded'] /
                        stats['wickets']) if stats['wickets'] > 0 else np.inf
    stats['economy'] = (stats['runs_conceded'] / stats['balls_bowled']
                        * 6) if stats['balls_bowled'] > 0 else np.inf
    stats['strike_rate'] = (
        stats['balls_bowled'] / stats['wickets']) if stats['wickets'] > 0 else np.inf
...
def aggregate_runs_by_match_year(df):
    ...
    df_agg = df.dropna(subset=['start_date'])
    if df_agg.empty:
        return pd.DataFrame()
    ...
    match_runs = df_agg.groupby(['year', 'match_id'])[
...
def plot_runs_distribution_ridge(df):
    ...
    years_sorted = sorted(match_runs_df['year'].unique())
    ...
    fig = px.violin(
        ...
        # Ensure years are ordered correctly
        category_orders={'year': years_sorted},
        box=False,         # Hide the inner box plot
        # Overlay helps but doesn't separate, px handles separation by y-category
        violinmode='overlay',
    ...
    fig.update_layout(
        # Set x-axis range, ensure at least 0-150
        xaxis=dict(
            range=[0, max(200, match_runs_df['runs_in_match'].max() * 1.05)]),
        yaxis_title="Year",
...
def plot_run_contribution(stats):
    ...
    data = {'Run Type': ['Other Runs (1s, 2s, 3s)', 'Runs from 4s', 'Runs from 6s'], 'Runs': [
        other_runs, runs_4s, runs_6s]}
    df_runs = pd.DataFrame(data)
    df_runs = df_runs[df_runs['Runs'] > 0]  # Only show slices with runs
    if df_runs.empty:
        return go.Figure().update_layout(title="Run Contribution (No Runs Scored)")
    fig = px.pie(df_runs, names='Run Type', values='Runs', title="Run Scoring Contribution",
                 hole=0.35, color_discrete_sequence=px.colors.qualitative.Pastel)
    fig.update_traces(textposition='inside',
                      textinfo='percent+label', pull=[0.05]*len(df_runs))
...
def plot_dismissal_pie(stats):
    ...
    labels = [k.capitalize() for k, v in dismissals_data.items()
              if v > 0 and k != 'not out']
    values = [v for k, v in dismissals_data.items() if v > 0 and k !=
              'not out']
    ...
    fig.update_layout(showlegend=True, legend_title_text='Dismissal Type',
...
def plot_sr_vs_order(df):
    ...
    # ensure match_id is not null
    df_plot = df_plot.dropna(
        subset=['order_seen', 'runs_scored', 'balls_faced', 'match_id'])
    # Only include innings where balls were faced
    df_plot = df_plot[df_plot['balls_faced'] > 0]
    if df_plot.empty:
    ...
    grouped = df_plot.groupby('order_seen').agg(
        ...
        # Count unique matches played at this order
    ...
    grouped['strike_rate'] = grouped['strike_rate'].replace(
        [np.inf, -np.inf, np.nan], 0)

    # --- MODIFIED: Calculate Average Runs per Match at that order ---
    grouped['avg_runs_per_match'] = grouped.apply(
        lambda row: row['total_runs'] /
        row['match_count'] if row['match_count'] > 0 else 0,
        axis=1
    )
    grouped['avg_runs_per_match'] = grouped['avg_runs_per_match'].replace(
        [np.inf, -np.inf, np.nan], 0)
    ...
    y_axis_max = max(
        100, max_sr * 1.1) if pd.notna(max_sr) and max_sr > 0 else 100
    ...
    if not grouped.empty:
        ...
        max_order = max(min_order, max_order) if pd.notna(
            max_order) else min_order
        orders = list(range(min_order, max_order + 1))
        xaxis_config = dict(tickmode='array', tickvals=orders,
                            ticktext=[str(o) for o in orders])
    else:
        xaxis_config = {}
...
def plot_bowler_timeline(df, aggregation_period='1Y'):  # Callers will pass 1Y
    ...
    df_resampled = df_time.resample(aggregation_period).agg(
        ...
        runs_conceded=('runs_conceded', 'sum'),
        balls_bowled=('balls_bowled', 'sum'),
        # Count distinct matches *with bowling* in period
        matches=('match_id', 'nunique')
    ).reset_index()

    df_resampled['economy_rate'] = df_resampled.apply(
        lambda row: (row['runs_conceded'] / row['balls_bowled']
                     * 6) if row['balls_bowled'] > 0 else np.inf,
        axis=1
    )
    df_resampled['economy_rate'] = df_resampled['economy_rate'].replace(
        [np.inf, -np.inf], np.nan)
    df_resampled = df_resampled[(df_resampled['wickets_taken'] > 0) | (
        df_resampled['balls_bowled'] > 0)]
    ...
    econ_upper_limit = max(
        15, max_econ * 1.2) if pd.notna(max_econ) and max_econ > 0 else 15

    fig.update_layout(
        title=f"Wickets & Economy Over Time (Aggregated per {aggregation_period})",
        xaxis_title="Period Start",
        yaxis=dict(title=dict(text="Wickets Taken", font=dict(
            color="#1f77b4")), tickfont=dict(color="#1f77b4")),
        yaxis2=dict(title=dict(text="Economy Rate", font=dict(color="#ff7f0e")), tickfont=dict(
            color="#ff7f0e"), anchor="x", overlaying="y", side="right", range=[0, econ_upper_limit]),
...
def plot_wicket_types_pie(stats):
    ...
    data = {'Wicket Type': ['Bowled', 'LBW', 'Other (Caught, Stumped, etc.)'], 'Count': [
        bowled, lbw, other]}
    df_wickets = pd.DataFrame(data)
    # Only show types with wickets
    df_wickets = df_wickets[df_wickets['Count'] > 0]
    if df_wickets.empty:
        # Changed message slightly
        return go.Figure().update_layout(title="Wicket Types (No Wickets Recorded)")
    fig = px.pie(df_wickets, names='Wicket Type', values='Count', title="Wicket Types Breakdown",
                 hole=0.35, color_discrete_sequence=px.colors.qualitative.Set2)
    fig.update_traces(textposition='inside',
                      textinfo='percent+value', pull=[0.05]*len(df_wickets))
    fig.update_layout(showlegend=True, legend_title_text='Wicket Type',
                      margin=dict(t=50, b=0, l=0, r=0))
    return fig


# Callers will pass 1Y
def plot_dot_ball_percentage_timeline(df, aggregation_period='1Y'):
    ...
    if 'balls_bowled' not in match_bowling_stats.columns:
        match_bowling_stats['balls_bowled'] = 0
    else:
        match_bowling_stats['balls_bowled'] = match_bowling_stats['balls_bowled'].fillna(
            0)

    if 'dot_balls' not in match_bowling_stats.columns:
        match_bowling_stats['dot_balls'] = 0
    else:
        match_bowling_stats['dot_balls'] = match_bowling_stats['dot_balls'].fillna(
            0)
    ...
    df_resampled['dot_ball_percentage'] = df_resampled.apply(
        lambda row: (row['dot_balls'] / row['balls_bowled']
                     * 100) if row['balls_bowled'] > 0 else 0,
        axis=1
    )
    df_resampled['dot_ball_percentage'] = df_resampled['dot_ball_percentage'].replace(
        [np.inf, -np.inf, np.nan], 0)
...
def layout():
    ...
    return dbc.Container([
        dbc.Row(dbc.Col(html.H1("🏏 Player Performance Analysis"),
                width=12), className="mb-4 mt-2"),
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                dcc.Dropdown(id='pa-player-dropdown', options=[],
            ...
            dbc.Col([
                ...
                dcc.RadioItems(
                    id='pa-role-radio',
                    options=[{'label': 'Batsman', 'value': 'Batsman'}, {
        ...
        dbc.Row([
            # Match Type Dropdown
            dbc.Col([
                dbc.Label("Filter by Match Type:",
                          html_for='pa-match-type-dropdown'),
                dcc.Dropdown(id='pa-match-type-dropdown', options=[],
                             value=DEFAULT_ALL_VALUE, clearable=False)
            ], width=12, md=6, lg=4, className="mb-3"),

            # Venue Dropdown
            dbc.Col([
                dbc.Label("Filter by Venue:", html_for='pa-venue-dropdown'),
                dcc.Dropdown(id='pa-venue-dropdown', options=[],
                             value=DEFAULT_ALL_VALUE, clearable=False)
            ], width=12, md=6, lg=4, className="mb-3"),
        ...
        # Output Area
        dbc.Row(dbc.Col(
            dcc.Loading(id="pa-loading-output",
                        children=[html.Div(id='pa-stats-output-area')]),
            width=12
        ))
...
@callback(
    ...
    Output('pa-date-range-picker', 'min_date_allowed'),
    # Set max date for picker
    Output('pa-date-range-picker', 'max_date_allowed'),
    # Potentially set default start
    Output('pa-date-range-picker', 'start_date'),
    # Potentially set default end
    Output('pa-date-range-picker', 'end_date'),
    # Input: Data from the central store
    ...
    # State: Current start date in picker
    State('pa-date-range-picker', 'start_date'),
    # State: Current end date in picker
    State('pa-date-range-picker', 'end_date'),
)
def update_player_analysis(player_data_json, selected_player, selected_role,
    ...
    if not player_data_json and triggered_id == 'player-analysis-store':
        print("No data found in player-analysis-store on initial load.")
        no_data_alert = dbc.Alert(
            "Error: Player analysis data could not be loaded.", color="danger")
        return no_data_alert, [], None, [], [], None, None, None, None
    elif not player_data_json:
        print(
            "Data store is empty, but trigger was not the store itself. Preventing update.")
        no_data_alert = dbc.Alert(
            "Error: Player analysis data is missing.", color="danger")
    ...
    try:
        ...
        # Assuming 'split' was used
        df_page_data = pd.read_json(player_data_json, orient='split')
        print(f"Data deserialized successfully. Shape: {df_page_data.shape}")
        if 'start_date' in df_page_data.columns:
            df_page_data['start_date'] = pd.to_datetime(
                df_page_data['start_date'], errors='coerce')
            ...
            if rows_dropped > 0:
                print(
                    f"Warning: Dropped {rows_dropped} rows with invalid 'start_date' values.")
            ...
            print(
                "Warning: 'start_date' column missing. Date filtering and timeline plots disabled.")
        ...
        error_alert = dbc.Alert(
            f"Error processing player data: {e}", color="danger")
    ...
    triggered_by_store = triggered_id == 'player-analysis-store'
    options_are_empty = not current_player_options

    min_data_date = df_page_data['start_date'].min(
    ) if 'start_date' in df_page_data.columns and not df_page_data.empty else None
    max_data_date = df_page_data['start_date'].max(
    ) if 'start_date' in df_page_data.columns and not df_page_data.empty else None
    min_date_str = min_data_date.strftime(
        '%Y-%m-%d') if pd.notna(min_data_date) else None
    max_date_str = max_data_date.strftime(
        '%Y-%m-%d') if pd.notna(max_data_date) else None
    ...
    if triggered_by_store or options_are_empty:
        ...
        if 'name' in df_page_data.columns:
            ...
            if not player_opts:  # No valid players
                ...
                # Set to first valid player
                player_val = player_opts[0]['value']
            elif current_selection not in player_choices:  # Previous selection is no longer valid
                # Reset to first valid player
                player_val = player_opts[0]['value']
            else:  # Keep current selection if valid
                player_val = current_selection
        else:
            player_opts = [
                {'label': "Data Error (Name missing)", 'value': "", 'disabled': True}]
            player_val = None

        default_mt_opts = [
            {'label': 'All Match Types', 'value': DEFAULT_ALL_VALUE}]
        if 'match_type' in df_page_data.columns:
            match_types = sorted(
                [str(mt) for mt in df_page_data["match_type"].dropna().unique()])
            # Filter out common string representations of NaN/None
            match_type_opts = default_mt_opts + \
                [{'label': mt, 'value': mt} for mt in match_types if mt.lower() not in [
                    'nan', 'none', 'null', '']]
        else:
            match_type_opts = default_mt_opts
        ...
        if 'venue' in df_page_data.columns:
            venues = sorted([str(v)
                            for v in df_page_data["venue"].dropna().unique()])
            # Filter out common string representations of NaN/None
            venue_opts = default_v_opts + [{'label': v, 'value': v}
                                           for v in venues if v.lower() not in ['nan', 'none', 'null', '']]
        else:
        ...
        print(
            f"Options populated. Player default/current: {player_val}, Date Range: {min_date} to {max_date}")
        ...
        if triggered_by_store and selected_player is None:
            # Use the determined player for the rest of the function run
            selected_player = current_player_for_initial_run
    ...
    if 'start_date' in filtered_df.columns and current_start_date_str and current_end_date_str:
        try:
            ...
            if pd.notna(start_dt) and pd.notna(end_dt):
                print(
                    f"Filtering by Date Range: {start_dt.date()} to {end_dt.date()}")
    ...
    if current_match_type != DEFAULT_ALL_VALUE and 'match_type' in filtered_df.columns:
        ...
        filtered_df = filtered_df[filtered_df['match_type'].astype(
            str) == str(current_match_type)]
    ...
    if current_venue != DEFAULT_ALL_VALUE and 'venue' in filtered_df.columns:
        ...
        filtered_df = filtered_df[filtered_df['venue'].astype(
            str) == str(current_venue)]

    # Check if empty *after* filtering
    if filtered_df.empty:
        filter_parts = []
        if current_match_type != DEFAULT_ALL_VALUE:
            filter_parts.append(f"Match Type='{current_match_type}'")
        if current_venue != DEFAULT_ALL_VALUE:
            filter_parts.append(f"Venue='{current_venue}'")
        if date_filter_applied:
            filter_parts.append(
                f"Dates='{start_dt.date()} to {end_dt.date()}'")
        filter_desc_str = ", ".join(
            filter_parts) if filter_parts else "the selected criteria"
    ...
    output_content = []
    date_range_desc_str = f" ({start_dt.date()} to {end_dt.date()})" if date_filter_applied and pd.notna(
        start_dt) else " (All Selected Dates)"
    ...
    if selected_role == 'Batsman':
        ...
        batsman_stats = calculate_batsman_stats(
            filtered_df.copy())  # Pass copy

        if batsman_stats.get('innings', 0) == 0:
            no_batting_alert = dbc.Alert(
                f"No batting innings found for {current_player} with the selected filters.", color="info")
            output_content = [
            ...
            summary_cards = dbc.Row([
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Matches", className="card-title"), html.P(
                    f"{batsman_stats.get('matches', 0)}")])), width=6, sm=4, md=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Innings", className="card-title"), html.P(
                    f"{batsman_stats.get('innings', 0)}")])), width=6, sm=4, md=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Runs", className="card-title"), html.P(
                    f"{batsman_stats.get('runs', 0):,}")])), width=6, sm=4, md=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Average", className="card-title"), html.P(f"{batsman_stats.get('average', 0):.2f}" if np.isfinite(
                    batsman_stats.get('average', 0)) else 'N/A')])), width=6, sm=6, md=3, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Strike Rate", className="card-title"), html.P(
                    f"{batsman_stats.get('strike_rate', 0):.2f}")])), width=6, sm=6, md=3, className="mb-2"),
            ], className="mb-3 text-center")

            # Generate Plots using the filtered data
            fig_ridge = plot_runs_distribution_ridge(
                filtered_df)  # Requires 'start_date'
            # Requires 'order_seen', 'match_id'
            fig_sr_order = plot_sr_vs_order(filtered_df)
            fig_runs_pie = plot_run_contribution(batsman_stats)
            fig_dismissal_pie = plot_dismissal_pie(
                batsman_stats)  # Requires 'out_kind'
            ...
            output_content = [
                ...
                summary_cards,
                html.Hr(),
                dbc.Row([dbc.Col(dcc.Graph(figure=fig_ridge), width=12)],
                        className="mb-4"),
                dbc.Row(
                    [dbc.Col(dcc.Graph(figure=fig_sr_order), width=12)], className="mb-4"),
                dbc.Row([
                    dbc.Col(dcc.Graph(figure=fig_runs_pie),
                            width=12, md=6, className="mb-3"),
                    dbc.Col(dcc.Graph(figure=fig_dismissal_pie),
                            width=12, md=6, className="mb-3"),
                ]),
            ]
    ...
    # --- Role: Bowler ---
        ...
        if bowler_stats.get('innings_bowled', 0) == 0:
            no_bowling_alert = dbc.Alert(
                f"No bowling innings found for {current_player} with the selected filters.", color="info")
            output_content = [
            ...
            summary_cards = dbc.Row([
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Matches Plyd", className="card-title"), html.P(
                    f"{bowler_stats.get('matches', 0)}")])), width=6, sm=4, lg=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Inns Bowled", className="card-title"), html.P(
                    f"{bowler_stats.get('innings_bowled', 0)}")])), width=6, sm=4, lg=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Wickets", className="card-title"), html.P(
                    f"{bowler_stats.get('wickets', 0)}")])), width=6, sm=4, lg=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Average", className="card-title"), html.P(f"{bowler_stats.get('average', 0):.2f}" if np.isfinite(
                    bowler_stats.get('average', 0)) else 'N/A')])), width=6, sm=4, lg=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Economy", className="card-title"), html.P(f"{bowler_stats.get('economy', 0):.2f}" if np.isfinite(
                    bowler_stats.get('economy', 0)) else 'N/A')])), width=6, sm=4, lg=2, className="mb-2"),
                dbc.Col(dbc.Card(dbc.CardBody([html.H5("Strike Rate", className="card-title"), html.P(f"{bowler_stats.get('strike_rate', 0):.2f}" if np.isfinite(
                    bowler_stats.get('strike_rate', 0)) else 'N/A')])), width=6, sm=4, lg=2, className="mb-2"),
            ], className="mb-3 text-center")

            # Generate Plots using the filtered data
            fig_timeline_bowler = plot_bowler_timeline(
                filtered_df, aggregation_period=BOWLER_AGG_PERIOD)  # Requires 'start_date'
            fig_dot_ball_timeline = plot_dot_ball_percentage_timeline(
                # Requires 'start_date', 'dot_balls_as_bowler'
                filtered_df, aggregation_period=BOWLER_AGG_PERIOD)
            # Requires 'bowled_done', 'lbw_done' etc. from stats calculation
            fig_wicket_pie = plot_wicket_types_pie(bowler_stats)
            ...
            output_content = [
                ...
                html.Hr(),
                dbc.Row(
                    [dbc.Col(dcc.Graph(figure=fig_timeline_bowler), width=12)], className="mb-3"),
                dbc.Row(
                    [dbc.Col(dcc.Graph(figure=fig_dot_ball_timeline), width=12)], className="mb-3"),
                dbc.Row([dbc.Col(dcc.Graph(figure=fig_wicket_pie),
                        width=12, md=6, className="mb-3")])  # Center pie maybe?
            ]
